...
- [x] Rewrite or trim generic "Why Choose Studio37" grids on local pages so each proof point uses a concrete operational detail.
...
- [x] Review inconsistent public metrics like 500+ sessions, 1000+ sessions, and happy-client counts and either source them or remove them.
- [x] Replace public em dashes that create avoidable polish issues in short marketing copy.
- [x] Keep the homepage hero image on hold until Christian discusses the direction with his partner.
- [x] Discuss newsletter popup strategy after the mechanical AI-tell cleanup is complete.
- [x] Homepage Studio37 identity pass: make the first viewport feel more authored and specific to Studio37 without adding generic marketing fluff.
- [x] Service hierarchy pass: make wedding, portrait, commercial, event, proposal, and local service cards/pages feel more distinct while staying in one visual system.
- [x] CTA language pass: reduce repeated generic labels and map calls-to-action to visitor intent, such as checking wedding availability, planning a portrait session, requesting private examples, or comparing coverage.
- [x] Operational proof pass: add small real-business details where helpful, including service area, two-photographer support, delivery expectations, planning help, and weather/location logistics.
- [x] Mobile spacing and rhythm pass: tighten pages where mobile sections feel too evenly padded, too airy, or tiring to scroll.
- [x] Branded image-loading pass: extend intentional placeholders/fallback treatments to image-heavy sections so slow loads never look unfinished.
- [x] Section label pass: replace selected generic labels like Services, Recent Work, and Why Choose Us with more specific Studio37 language where it improves clarity.
- [x] Blog index editorial pass: strengthen featured-post hierarchy, card rhythm, categories, and photographic texture so the archive feels less stock-CMS.
- [x] Local page authenticity pass: add more useful local context such as venues, location conditions, travel radius, session fit, and city-specific planning notes without over-SEOing.
- [x] Conversion path clarity pass: make pricing, package recommender, booking, and portfolio request feel like distinct paths for different visitor intents.
